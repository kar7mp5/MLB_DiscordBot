...
class RockPaperScissors(discord.ui.Select):
    def __init__(self) -> None:
        options = [
            ...
            discord.SelectOption(label="Paper", description="You choose paper.", emoji="🧻"),
        ]
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        result_embed.set_author(name=interaction.user.name, icon_url=interaction.user.display_avatar.url)
...
class Fun(commands.Cog, name="fun"):
    ...
    async def coinflip(self, context: Context) -> None:
        ...
        if buttons.value == result:
            embed = discord.Embed(description=f"Correct! You guessed `{buttons.value}` and I flipped the coin to `{result}`.", color=0xBEBEFE)
        else:
